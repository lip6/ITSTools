...
public class SiphonComputer {
	// computes a list of integers corresponding to a subset of places, which form
	// an initially empty syphon.
	...
	public static Set<Integer> computeEmptySyphon(IntMatrixCol flowPT, IntMatrixCol flowTP, List<Integer> marks) {
		...
		// Logger.getLogger("fr.lip6.move.gal").info("Computed a system of
		// "+sr.getPnames().size()+"/"+ srori.getPnames().size() + " places and
		// "+sr.getTnames().size()+"/"+ srori.getTnames().size() + " transitions for
		// Syphon test. " + (System.currentTimeMillis()-time) +" ms");
		if (keepP.isEmpty()) {
			// fail
			return new HashSet<>();
		} else {
			// okay so we have a siphon here
			System.out.println("Deduced a syphon composed of " + keepP.size() + " places in "
