...
/* Pipe 2 */
chan P2_data_out = [1] of { data_t };
chan P2_req_out = [1] of { req_t };
...
chan WM2_data_out = [1] of { data_t };
chan WM2_req_out = [1] of { req_t };

chan WM2_from_task = [1] of {commande_t};
chan WM2_to_task = [1] of {commande_t};
...
chan WS2_data_out = [1] of { data_t };
chan WS2_req_out = [1] of { req_t };

chan WS2_from_task = [1] of {commande_t};
chan WS2_to_task = [1] of {commande_t};
...
progress: do 
  :: atomic { WS2_data_out?data; WS2_req_out?req; }
     atomic { P1_data_out!data; P1_req_out!req; }
  od;
}

active[1] proctype Pipe1()
{
  data_t data;
  req_t req;
  ...
  atomic{
    ...
    P2_data_out!data; P2_req_out!req; 
  }

progress: do 
  :: atomic { WM1_data_out?data; WM1_req_out?req; }
     atomic { P2_data_out!data; P2_req_out!req; }
  od;
}
...
debut: do
  :: atomic { WM2_data_out?data; WM2_req_out?req; }
     if
...
active[1] proctype WrapperMaitre1()
{
/*  xr data_in; xs data_out;
  xr req_in; xs req_out;
  xs to_task;*/
...
debut: do
  :: (empty(WM2_from_task)) -> /* pas de requette sur from_task */
       atomic { P2_data_out?data; P2_req_out?req; }
       atomic { WM2_data_out!data; WM2_req_out!req; }
  :: (nempty(WM2_from_task)) -> /* requette sur from_task */
debut_req: do
       :: atomic { P2_data_out?data; P2_req_out?req; }
	  if 
	  :: (req.free == true) -> break; /* requette libre */
	  :: (req.free != true) -> atomic { WM2_data_out!data; WM2_req_out!req; }
	  fi;
       od;
       
       /* création de la requette */
       atomic{
       WM2_from_task?req.cde;
       req.answer = false;
       req.free = false;
       req.src = 1;
       /* on sauvegarde qlqs infos */
       ...
       dest = req.cde.dest;
       /* on envoie la requette sur le bus */
       WM2_data_out!data; WM2_req_out!req;
       }
	  ...
	  if 
	  :: ((!req.free) && req.answer && (req.src == 1) ) -> 
	       break;
	  :: else ->atomic { WM2_data_out!data; WM2_req_out!req; }
	  fi;
       od;
	 ...
	 WM2_data_out!data; WM2_req_out!req;
       ...
       :: else ->
		 ...
		 :: (!data.free &&
		    ((data.lg0 + (data.lg1*2) + (data.lg2*4)) == (cpt+1)) &&
		    (data.type_dest == WM) && (data.id_dest) == 1) ->
	       	      atomic {
	                WM2_to_task!req.cde;
		        cpt++;
		        data.free = true;
	 ...
	 :: ((dir == WRITE) || (lg == 0)) -> /* si c'est un WRITE ou READWRITE */
	      ...
	      :: ((cpt < lg) || (lg == 0 && cpt == 0)) ->
		   ...
		   :: (empty(WM2_from_task)) -> 
		        atomic { P2_data_out?data; P2_req_out?req; }
			atomic { WM2_data_out!data; WM2_req_out!req; }
		   :: (nempty(WM2_from_task)) ->
		        do
			:: atomic { P2_data_out?data; }
			   if
			   :: (data.free) ->
			        atomic {
				  WM2_from_task?req.cde;
...
active [1] proctype WrapperEsclave1()
{
/*  xr data_in; xs data_out;
  xr req_in; xs req_out;
  xs to_task;*/
...
debut: do
  :: atomic { WS1_data_out?data; WS1_req_out?req; }
     if
     :: ((!req.free) && (!req.answer) && (req.cde.dest == 1) &&
	(!locked || req.src == src)) ->
          /* on sauvegarde qlqs infos */
          atomic {
	  lg = req.cde.lg0 + (req.cde.lg1 * 2) + (req.cde.lg2 * 4);
	  ...
	  /* on envoie l'acquitement sur le bus REQ */
	  req.answer = true;
	  WS2_data_out!data; WS2_req_out!req; 
	  }
	  ...
	  :: else -> skip;
	     ...
	     :: ((cpt < lg) || (lg == 0 && cpt == 0)) -> 
	          ...
	          :: (empty(WS2_from_task)) -> 
		       atomic { WS1_data_out?data; WS1_req_out?req; }
		       atomic { WS2_data_out!data; WS2_req_out!req; }
	          :: (nempty(WS2_from_task)) ->
		       do /* on attend que le bus DATA soit libre */
		       :: atomic { WS1_data_out?data;}
		          if
		          :: (data.free) ->
			       atomic {
			         WS2_from_task?req.cde;
				 cpt++;
				 data.free = false;
				 data.lg0 = cpt;
				 data.lg1 = cpt / 2;
	     ...
	     od;
	      ...
	      :: ((cpt < lg) || (lg == 0 && cpt == 0)) -> 
	           ...
	           if
	           :: (!data.free && 
		      (data.type_dest == WE) && (data.id_dest == 1) &&
		      ((data.lg0 + (data.lg1*2) + (data.lg2*4)) == cpt+1)) ->
		        atomic {
			WS2_to_task!req.cde;
		        cpt++;
     ...
     :: else -> atomic { WS2_data_out!data; WS2_req_out!req; }
     fi;
  od;
}
...
debut: do 
  ...
  od;
        ...
        active[1] proctype TacheMaitre1()
{
//  xr to_task;
...
active [1] proctype TacheEsclave1()
{
//  xr to_task;
